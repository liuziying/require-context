...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    value: function apply(op) {
      ...
      // 仅查找当前文件中第一个匹配的$require-context方法
      ...
      var requireContextStr = res[0];
      var useDefault = false;
      requireContextStr = requireContextStr.replace(/\'/gi, '"'); // 参数统一转为双引号
      var buildArgs = requireContextStr.match(/\"[^\"]+\"/gi); // 提取参数，结果带有双引号
      if (!buildArgs || !buildArgs.length) {
        // 参数为空，不合法
        op.next();
        return;
      } else if (buildArgs.length > 2) {
        useDefault = '"true"' === buildArgs[2];
      }
